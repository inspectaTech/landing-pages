   ...
   * @category Server
   * @subcategory tools
   * @desc creates a unique alias for each data item
   * @see [setup (linkback)]{@link module:OAServer-controllers-setup}
   * @see [oas-check-make-email (linkback)]{@link module:oas-check-make-email}
  ...
  /**
   * @callback oas-alias-maker
   * @desc creates a unique alias for each data item
   * @param  {object}  item the item that needs an alias
   * @return {void}
   */
  const alias_maker = async function(item)
  {
    ...
    if(display_console || false) console.log(chalk.yellow("[alias_maker] item"),item);
  ...
  const testAlias = async function(tObj)
  {
    try {
      ...
      if(display_console || false) console.log(chalk.cyan("[match]"),match);
      ...
      if(mode == "add"){
        ...
        if(display_console || false) console.log(chalk.yellow("[is_ok]"),is_ok);
      }else{
      ...
      if(is_ok){
        // if the similar one
        // return prep_alias;
        if(display_console || false) console.log(chalk.yellow("[prep_alias value]"),prep_alias);
      }else{
        ...
        if(mode == "edit"){
          ...
          if(display_console || false) console.log(chalk.magenta("[item alias length]"),similar.length);
      ...
      if(display_console || false) console.error(chalk.red("[testAlias]"),"an error has occured",e);
    }
  ...
  const iUN_alias = function (pA) {
    ...
    if(display_console || false) console.log(chalk.yellow("[prep_alias value] iUN"),prep_alias);
    return prep_alias;
  }
  ...
  const setDefaultAlias = async function (items) {
    try {
      ...
      for(value of items){
        ...
        if(display_console || false) console.log(chalk.yellow("[prep_alias]"),prep_alias);
        let ils = await Item.find({user_id: value.user_id, alias: {"$regex": alias_title,"$options": "i"}}).lean();//"$regex": "Alex", "$options": "i"

        if(display_console || false) console.log(chalk.magenta("[item alias length]"),ils.length);

        if(ils && ils.length < 2){
          value.alias = prep_alias.alias;
          value.path = prep_alias.path;
          if(display_console || false) console.log(chalk.yellow("[alias value]"),value);
          ...
          value.alias = prep_alias.alias;
          value.path = prep_alias.path;
          if(display_console || false) console.log(chalk.yellow("[alias value] iUN"),value);
          await Item.findOneAndUpdate({_id: value._id}, value, { new: true });
        }// else
      ...
      if(display_console || false) console.error(chalk.red("[set default alias]"),"an error has occured",e);
    }
  }// setDefaultAlias
