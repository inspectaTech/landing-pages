...
try {
  ...
  if(display_console || false) console.log(chalk.yellow(`[check_make_binder] accessed`));
  if(display_console || false) console.log(chalk.magenta(`[check_make_binder] user`),user);
  if(display_console || false) console.log(chalk.blue(`[check_make_binder] binder`),binder);
  ...
  if(rtn && requested_binder){
    if(display_console || false) console.log(chalk.green(`[check_make_binder] ${binder} requested_binder`),requested_binder);
    return requested_binder;
  }

  if(display_console || false) console.log(chalk.yellow(`[check_make_binder] ${binder} requested_binder`),requested_binder);
  if(!requested_binder){

    // here requested_binder == null
    if(display_console || false) console.log(chalk.magenta(`[check_make_binder] ${binder} no requested_binder was found`));
    ...
    if(display_console || false) console.log(chalk.green(`[check_make_binder] ${binder} newItem`),newItem);

    let update_obj = await alias_maker(newItem);

    if(display_console || false) console.log(chalk.green(`[check_make_binder] ${binder} update_obj`),update_obj);
  ...
  if(display_console || false) console.log(chalk.red(`[check-make-binder] ${binder} an error occured`),e);
}
