  ...
  const {check_make_item} = require('./check-make-item');
  ...
  const check_make_preset = async ({user, method, type = "info", category = "preset", title = "default preset"}, rtn) => {
    ...
    try {
      ...
      if(!exists(requested_preset)){
        ...
        let item_preset;
        // make sure the project_item doesn't already have a preset_item
        if(typeof project_item.preset_id != "undefined" && is_objectId_valid(project_item.preset_id)){
          item_preset = await Item.findOne({_id: project_item.preset_id});
        }
        // update the project items preset_id if no other preset_item is available
        if(!item_preset){
          let set = {$set: {}};
