...
const basicProfile = props => {
  // props.children;
  // NOW: profile template
  console.log(`[basicProfile] PRESET_DATA`,PRESET_DATA);
  let iUN =  Math.round(Math.random() * 10000);
  let name = `basicProfile`;

  let data = PRESET_DATA.data;
  ...
  useEffect(() => {
    ...
    let image_array = sort_preset({data, type:'picture'});

    if(image_array.length > 0){
      let image_data = image_array[0];
  ...
  let display_email = (email_array.length > 0) ? (
    <div id={`${name}_email_cont_${iUN}`} className={`${name}_email_cont_${iUN} ${name}_email_cont`}>
    {
      email_array.map((item, ndx) => {
        return <h4 key={`bp_${iUN}_${ndx}`}>{item.core_data}</h4>
      })
    }
    </div>
  ) : null;
