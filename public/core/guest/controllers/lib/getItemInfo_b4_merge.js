...
const mongoose = require('mongoose');
  ...
  const getItemInfo = async function(req, res)
  {
    ...
    try {
        ...
        // LATER: get unpublished if owner otherwise only get published - indicate unpublished with icon and/or color
        if(mode == "none"){
          rows = await Item.find({ user_id: item_data.user_id, ancestor: {$exists: true, $eq:item_ancestor}, type: item_type, published: true }).lean();
        }else {
          page = parseInt(page);
          limit = parseInt(limit);
          let start_at = (page - 1) * limit;
          rows = await Item.find({ user_id: item_data.user_id, ancestor: {$exists: true, $eq:item_ancestor}, type: item_type, published: true }).skip(start_at).limit(limit).lean();
        }
