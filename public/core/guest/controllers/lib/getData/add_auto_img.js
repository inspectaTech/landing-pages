...
const get_auto_img = async (item) => {
  ...
  if ((!collections.includes(item.data_type) && !item.container) || exists(item?.img_url)) return;
...
const add_auto_img = async ({rows}) => {
  ...
  for (let item of rows) {
    ...
    if ((collections.includes(item.data_type) || item.container == true) && !exists(item?.img_url)){
