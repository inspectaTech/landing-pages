...
const getPreviewData = async function (req, res)
{
  ...
  try {
    ...
    if(display_console) console.log("[guest/getPreviewData][req/post req.body]",req.body);
    let obj = req.body;
    let task = obj.task;
    ...
    if(display_console) console.log(`[guest/getPreviewData][url obj] = ${JSON.stringify(url_obj)}`);
    ...
    switch (site_type) {
      ...
      get_youtube_data({url},(error,ret_obj) => {
        ...
        if(error){
          console.log(chalk.red("[guest/getPreviewData] get_youtube_data an error occured"), error);
      ...
      get_site_data({url},(error,ret_obj) => {
        ...
        if(error){
          console.log(chalk.red("[guest/getPreviewData] get_site_data an error occured"),error);
          return res.send({error:true, message:error});
        }
      ...
      console.log(chalk.red("[guest/getPreviewData] overall an error occured "),e);
