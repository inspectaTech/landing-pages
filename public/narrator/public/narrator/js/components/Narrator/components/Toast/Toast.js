    ...
    */
    ...
    render() {
      ...
      let close_btn = s.close ? (
        <div className={`${s.name}_toast_close_${s.iUN} toast_close d3-ico d3-bg d3-disc-outer d3-disc-bg d3-abs icon-cross`}
          onClick={(e) => {e.preventDefault(); this.remove_self()}}
        ></div>
      ) : null;

      return(
        <div className={`${s.name}_toast_cont_${s.iUN} toast_cont d3_toast ${toast_class} ${show_card} d3-slide-in-bottom d3-fade-in-quick`}
          ref={this.toast_cont}
          data-comp={`Toast`}
          onClick={(e) => {e.preventDefault(); this.remove_self()}} >
          {close_btn}
